...
namespace Arkivverket.Arkade.Core.Tests.Base.Addml.Processes
{
    public class A_33_ControlOrganizationNumberTest : LanguageDependentTest
    {
        ...
        public void ShouldReportErrorWhenInvalidOrganizationNumbersAreFound()
        {
            ...
            test.Run(flatFile);
            test.Run(new Field(fieldDefinition, "914994780"), 1); // ok
            test.Run(new Field(fieldDefinition, "914994781"), 1); // invalid checksum
            test.Run(new Field(fieldDefinition, "91499478"), 1); // invalid length
            test.Run(new Field(fieldDefinition, "91499478A"), 1); // invalid characters
            test.EndOfFile();
